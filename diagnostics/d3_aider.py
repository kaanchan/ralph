...
def run_aider(tmp, target, extra_args=(), base_args=("--auto-commits",),
    ...
    cmd = [PYTHON, "-m", "aider",
           ...
           *base_args,
           "--model", MODEL,
           "--message", "Replace this file content with exactly one line: x = 42",
           *extra_args, target]
...
if __name__ == "__main__":
    ...
    # Run D first — it's the recommended path. If it works, we have what we need.
    ...
    log(f"\n  ── verdict ──────────────────────────────────────")
    log(f"  Variant D (--no-auto-commits)    : {'PASS' if d_ok else 'FAIL'}  <- RECOMMENDED")
    log(f"  Variant A (pre-committed)        : {'PASS' if a_ok else 'FAIL'}")
    log(f"  Variant B (--no-dirty-commits)   : {'PASS' if b_ok else 'FAIL'}")
    log(f"  Variant C (default / dirty)      : {'PASS' if c_ok else 'FAIL (expected)'}")
    log("")
    if d_ok:
        log("  >> USE VARIANT D in executor.py: pass --no-auto-commits and have")
        log("     RALPH commit changes itself. This is Aider's documented workflow")
        log("     for agentic/scripted use and avoids the hang entirely.")
    elif a_ok:
        log("  >> Fallback: pre-commit the placeholder before invoking Aider.")
    else:
        log("  >> Both recommended workflows failed. Check Ollama heartbeat output.")

    overall_ok = d_ok or a_ok
    log(f"\n  {'PASS (workaround found)' if overall_ok else 'FAILED'}  (total {ts()})\n")
